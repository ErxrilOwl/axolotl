...
const props = defineProps<{
  departments?: { data: DepartmentRow[]; meta: any }
  search?: string
  status?: string
}>()

const breadcrumbs = [{ title: 'Departments', href: '/departments' }]
...
const onSearch = () => {
  router.get('/departments', form.data(), { preserveState: true })
}
...
const onDelete = () => {
  ...
  router.delete(`/departments/${selectedId.value}`, {
    preserveScroll: true,
    onSuccess: () => {
...
<template>
  ...
  <BaseBreadcrumb title="Departments" :breadcrumbs="breadcrumbs" />
  ...
  <div class="flex h-full flex-1 flex-col gap-4 py-4">
    <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
      ...
      <div class="w-full md:w-auto">
        <Link href="/departments/create" class="w-full md:w-auto">
      ...
      <DataTable>
        <DataTHead>
          <DataTr>
            ...
            <DataTh class="w-2/12"><p class="text-xs font-medium text-gray-500 dark:text-gray-400">Code</p></DataTh>
            <DataTh class="w-2/12"><p class="text-xs font-medium text-gray-500 dark:text-gray-400">Users</p></DataTh>
            <DataTh class="w-2/12"><p class="text-xs font-medium text-gray-500 dark:text-gray-400">Status</p></DataTh>
            <DataTh class="w-2/12"><p class="text-xs font-medium text-gray-500 dark:text-gray-400">Actions</p></DataTh>
          </DataTr>
        </DataTHead>
        <DataTBody v-if="props.departments?.data.length">
          <DataTr v-for="department in props.departments?.data" :key="department.id">
            <DataTd><p class="text-xs text-gray-500 dark:text-gray-400">{{ department.id }}</p></DataTd>
            <DataTd><p class="text-xs text-gray-500 dark:text-gray-400">{{ department.name }}</p></DataTd>
            <DataTd><p class="text-xs text-gray-500 dark:text-gray-400">{{ department.code }}</p></DataTd>
            <DataTd><p class="text-xs text-gray-500 dark:text-gray-400">{{ department.users_count }}</p></DataTd>
            <DataTd>
              <span
                v-if="department.fixed"
                class="rounded-full bg-blue-light-50 px-2 py-0.5 text-xs font-medium text-blue-light-600 dark:bg-blue-light-500/15 dark:text-blue-light-400"
              >
                Protected
              </span>
            ...
            <DataTd class="flex gap-2">
              ...
              <Button
                ...
                variant="destructive"
                type="button"
                @click="setToDelete(department.id)"
              >
        ...
        <DataTBody v-else>
          <DataTr>
            <DataTd :colspan="6">
              <p class="text-center text-xs text-gray-500 dark:text-gray-400">No departments found</p>
            </DataTd>
          </DataTr>
        </DataTBody>
      </DataTable>
      <Pagination :pagination="props.departments?.meta" />
    </div>
  </div>
</template>
