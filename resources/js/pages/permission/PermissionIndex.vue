...
const props = defineProps<{
  permissions?: { data: PermissionRow[]; meta: any }
  search?: string
  status?: string
}>()
...
const onSearch = () => {
  router.get('/permissions', form.data(), { preserveState: true })
}
...
<template>
  ...
  <BaseBreadcrumb title="Permissions" :breadcrumbs="breadcrumbs" />
  ...
  <div class="flex h-full flex-1 flex-col gap-4 py-4">
    <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
      ...
      <div class="w-full md:w-auto">
        <Link href="/permissions/create" class="w-full md:w-auto">
      ...
      <DataTable>
        <DataTHead>
          <DataTr>
            ...
            <DataTh class="w-5/12"><p class="text-xs font-medium text-gray-500 dark:text-gray-400">Name</p></DataTh>
            <DataTh class="w-3/12"><p class="text-xs font-medium text-gray-500 dark:text-gray-400">Roles</p></DataTh>
            <DataTh class="w-3/12"><p class="text-xs font-medium text-gray-500 dark:text-gray-400">Actions</p></DataTh>
          </DataTr>
        </DataTHead>
        <DataTBody v-if="props.permissions?.data.length">
          <DataTr v-for="permission in props.permissions?.data" :key="permission.id">
            <DataTd><p class="text-xs text-gray-500 dark:text-gray-400">{{ permission.id }}</p></DataTd>
            <DataTd><p class="text-xs text-gray-500 dark:text-gray-400">{{ permission.name }}</p></DataTd>
            <DataTd><p class="text-xs text-gray-500 dark:text-gray-400">{{ permission.roles_count }}</p></DataTd>
            <DataTd class="flex gap-2">
              <Link :href="`/permissions/${permission.id}/edit`">
                <Button variant="outline"><Pencil class="h-4 w-4" /></Button>
              </Link>
              <Button variant="destructive" type="button" @click="setToDelete(permission.id)">
        ...
        <DataTBody v-else>
          <DataTr>
            <DataTd :colspan="4">
              <p class="text-center text-xs text-gray-500 dark:text-gray-400">No permissions found</p>
            </DataTd>
          </DataTr>
        </DataTBody>
      </DataTable>
      <Pagination :pagination="props.permissions?.meta" />
    </div>
  </div>
</template>
